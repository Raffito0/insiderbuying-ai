...
var stackbit_config_default = (0, import_types.defineStackbitConfig)({
  ...
  contentSources: [
    new import_cms_git.GitContentSource({
      rootPath: "C:\\Users\\rafca\\OneDrive\\Desktop\\Toxic or Nah\\ryan_cole\\insiderbuying-site",
      contentDirs: ["content"],
      models: [
        /* ── PAGE MODELS ── */
        {
  ...
  siteMap: ({ documents }) => {
    return documents.filter((doc) => doc.modelName && ["HomePage", "AboutPage", "PricingPage", "HowItWorksPage", "FreeReportPage", "BlogPage", "MethodologyPage"].includes(doc.modelName)).map((doc) => ({
      stableId: doc.id,
      urlPath: doc.fields?.urlPath || "/",
      document: doc
    }));
  }
});
//# sourceMappingURL=stackbit.config.A4K2CG5H.cjs.map
