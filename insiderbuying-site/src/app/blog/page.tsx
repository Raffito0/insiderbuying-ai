  ...
  return (
    <div className="bg-[#fcf9f8] flex flex-col">
        ...
        <div className="max-w-[1200px] mx-auto px-[16px] md:px-[32px] flex items-center justify-between h-[60px] md:h-[71px] gap-[16px]">
          <div className="flex items-center gap-[12px]">
            <select
              ...
              className="h-[36px] px-[12px] text-[13px] border border-[#d1d6da] rounded bg-white text-[#1a1a1a]"
            >
          ...
          <div className="flex items-center bg-[#f6f3f2] rounded h-[36px] px-[14px] gap-[8px]">
            ...
            <input
              ...
              className="bg-transparent text-[14px] text-[#1a1a1a] placeholder:text-[#6b7280] outline-none w-[120px]"
            />
          </div>
        </div>
      </section>
      ...
      <section className="max-w-[1200px] mx-auto w-full px-[16px] md:px-[32px] pt-[32px] md:pt-[48px] pb-[48px]">
        {loading ? (
          <div className="text-center py-[48px] text-[#5c6670]">Loading articles...</div>
        ) : articles.length === 0 ? (
          <div className="text-center py-[48px] text-[#5c6670]">No articles found.</div>
        ) : (
          <>
            {/* Featured (first article) */}
            {articles.length > 0 && (
              <div className="flex flex-col md:flex-row gap-[20px] md:gap-[32px] md:items-center mb-[48px] md:mb-[80px]">
                <div className="w-full md:w-[387px] h-[200px] md:h-[217px] bg-[#e5e2e1] rounded-[4px] shrink-0 overflow-hidden">
                ...
                <div className="flex flex-col gap-[8px] md:gap-[11px]">
                  <div className="flex items-center gap-[8px]">
                    ...
                    <span className="text-[10px] font-medium text-[#5c6670]">{articles[0].ticker}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-[12px] mt-[8px]">
                    <span className="text-[12px] text-[#5c6670]">{articles[0].author_name}</span>
                    <span className="text-[12px] text-[#5c6670]">{formatDate(articles[0].published_at)}</span>
                    <span className="text-[12px] text-[#5c6670]">{Math.ceil((articles[0].word_count || 0) / 200)} min read</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[32px] md:gap-[48px]">
              {articles.slice(1).map((a) => (
                <div key={a.id} className="flex flex-col">
                  <div className="w-full aspect-[16/9] bg-[#e5e2e1] rounded-[4px] mb-[12px] overflow-hidden">
                    {a.hero_image_url && (
                  ...
                  <div className="flex items-center gap-[8px] mb-[4px]">
                    ...
                    <span className="text-[10px] font-medium text-[#5c6670] font-mono">{a.ticker}</span>
                  </div>
                  ...
                  <p className="text-[14px] font-normal leading-[20px] text-[#5c6670] mb-[8px] line-clamp-2">{a.meta_description}</p>
                  <div className="flex items-center gap-[8px] pt-[8px]">
                    <span className="text-[12px] text-[#5c6670]">{a.author_name}</span>
                    <span className="text-[12px] text-[#5c6670]">{formatDate(a.published_at)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-[12px] pt-[32px]">
              {page > 1 && (
                <button onClick={() => setPage(page - 1)} className="h-[46px] px-[24px] rounded border border-[#d1d6da] text-[14px] font-bold text-[#1a1a1a] hover:bg-[#f6f3f2]">
                  Previous
                </button>
              )}
              {articles.length === 12 && (
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row items-start lg:items-center justify-between gap-[24px] lg:gap-[64px]">
          ...
          {nlSubmitted ? (
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex flex-col sm:flex-row gap-[8px] w-full sm:w-auto shrink-0">
              <input type="email" required value={nlEmail} onChange={(e) => setNlEmail(e.target.value)} placeholder="Email Address" className="w-full sm:w-[300px] h-[48px] bg-white px-[24px] text-[16px] text-[#1a1a1a] placeholder:text-[#6b7280]" />
              <button type="submit" disabled={nlLoading} className="h-[48px] px-[32px] bg-white text-[16px] font-medium text-[#1c1b1b] hover:bg-white/90 disabled:opacity-50">
                {nlLoading ? "..." : "Subscribe"}
              </button>
            </form>
          )}
