...
const PLANS = [
  { name: "Free", desc: "Essential market monitoring.", price: "$0", features: ["Real-time alert feed", "Weekly digest email", "Basic transaction data"], border: "", btn: "border border-[var(--color-border)] text-[var(--color-text)]" },
  ...
  { name: "Investor", desc: "Maximum edge for serious investors.", price: "$89", features: ["Everything in Pro", "All deep dive reports included", "Monthly backtest report", "Priority support"], border: "", btn: "border border-[var(--color-border)] text-[var(--color-text)]" },
];
    ...
    <div className="bg-white">
      ...
      <section className="w-full py-[24px] bg-white overflow-hidden">
        <div className="flex gap-[48px] animate-[scroll_30s_linear_infinite] whitespace-nowrap">
          {[...LOGOS, ...LOGOS].map((logo, i) => (
            <span key={i} className="text-[13px] font-medium text-[var(--color-text)]/60 shrink-0">{logo}</span>
          ))}
        </div>
        ...
        <div className="max-w-[1154px] mx-auto">
          <div className="flex flex-col items-center lg:flex-row lg:items-center gap-[12px] lg:gap-[var(--gap-tight)] mb-[24px]">
            <div className="flex items-center gap-[8px] bg-[#00de16]/20 px-[12px] py-[4px] rounded-full">
              ...
              <span className="text-[12px] font-medium tracking-[0.5px] text-[var(--color-signal-green)]">Live</span>
            </div>
            <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] font-normal leading-[1.1] tracking-[0.5px] text-[var(--color-text)] text-center lg:text-left">Recent Insider Buys</h2>
          </div>
            ...
            {ALERTS.map((a, i) => (
              <div key={i} className={`flex items-center justify-between h-[112px] px-[32px] ${a.bg} ${i > 0 ? "border-t border-[var(--color-border)]" : ""}`}>
                <div className="flex items-center gap-[24px]">
                  <div className="w-[64px] h-[64px] rounded-full bg-[#d9d9d9] flex items-center justify-center text-[14px] font-semibold text-[var(--color-text-muted)]">
                    {a.name.split(" ").map(n => n[0]).join("")}
                  </div>
                  <div className="flex flex-col gap-[4px]">
                    <span className="text-[13px] font-normal text-[var(--color-text-muted)]">{a.title}</span>
                    <span className="font-[var(--font-montaga)] text-[18px] font-normal leading-[28px] text-[var(--color-text)]">{a.name}</span>
                  </div>
                </div>
                <span className="text-[24px] font-medium leading-[32px] text-[var(--color-text)]">{a.ticker}</span>
                <span className="text-[20px] font-medium text-[var(--color-signal-green)]">{a.amount}</span>
                <span className="text-[16px] font-normal text-[var(--color-text-secondary)]">{a.time}</span>
              </div>
            ))}
          </div>
          ...
          <div className="lg:hidden bg-[var(--color-bg-alt)] overflow-hidden mb-[var(--gap-tight)]">
            {ALERTS.map((a, i) => (
              <div key={i} className={`flex items-center gap-[12px] px-[var(--gap-tight)] py-[14px] ${i > 0 ? "border-t border-[var(--color-border)]" : ""}`}>
                <div className="w-[40px] h-[40px] rounded-full bg-[#d9d9d9] flex items-center justify-center text-[11px] font-semibold text-[var(--color-text-muted)] shrink-0">
                  {a.name.split(" ").map(n => n[0]).join("")}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-medium leading-[18px] text-[var(--color-text)] truncate">{a.name}</p>
                  <p className="text-[11px] font-normal text-[var(--color-text-muted)]">{a.title}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-[13px] font-semibold tracking-[0.5px] text-[var(--color-text)] font-[var(--font-mono)]">{a.ticker}</p>
                  <p className="text-[14px] font-semibold text-[var(--color-signal-green)]">{a.amount}</p>
                  <p className="text-[11px] font-normal text-[var(--color-text-muted)]">{a.time}</p>
                </div>
              </div>
            ))}
          </div>
          <p className="text-[16px] font-normal leading-[20px] text-[var(--color-text-muted)] text-center">Source: SEC Form 4 Filings. Data updated every 15 seconds.</p>
        </div>
      </section>
      ...
      <section id="how-it-works" className="w-full pt-[40px] px-[20px] md:px-[100px] pb-[var(--section-y-mobile)] md:pb-[var(--section-y)] bg-white">
        <div className="max-w-[1080px] mx-auto px-[32px]">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] font-normal leading-[1.1] tracking-[0.5px] text-[var(--color-text)] text-center mb-[40px] md:mb-[80px]">How It Works</h2>
          <div className="flex flex-col md:flex-row justify-between gap-[40px] md:gap-[63px]">
            {[
              ...
              <div key={s.title} className="w-full md:w-[296px] text-center">
                ...
                <h3 className="font-[var(--font-montaga)] text-[22px] font-normal leading-[28px] text-[var(--color-text)] mb-[8px]">{s.title}</h3>
                <p className="text-[16px] font-normal leading-[26px] text-[var(--color-text-secondary)]">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full py-[var(--section-y-mobile)] px-[20px] md:pt-[var(--section-y)] md:pr-[98px] md:pb-[var(--section-y)] md:pl-[98px] bg-[var(--color-bg-alt)]">
        <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] font-normal leading-[1.1] tracking-[0.5px] text-[var(--color-text)] max-w-[1084px] mx-auto mb-[32px] text-center lg:text-left">Why Insider Buying Matters</h2>
        <div className="max-w-[1084px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-[var(--gap-items)]">
          {STATS.map((s) => (
            <div key={s.value} className="bg-white pt-[15px] pr-[29px] pb-[30px] pl-[29px]">
              <p className="font-[var(--font-montaga)] text-[40px] lg:text-[48px] font-normal leading-[84px] tracking-[0.5px] text-[var(--color-text)]">{s.value}</p>
              <p className="font-[var(--font-montaga)] text-[20px] font-normal leading-[28px] text-[var(--color-text)] mb-[6px]">{s.label}</p>
              <p className="text-[16px] font-normal leading-[23px] text-[var(--color-text-secondary)] mb-[4px]">{s.desc}</p>
              <p className="text-[13px] font-normal leading-[23px] tracking-[0.5px] text-[var(--color-text-muted)]">{s.source}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="bg-[var(--color-bg-alt)] border-b border-black/10 px-[var(--gap-tight)] py-[20px] md:px-[32px] md:py-[32px]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-[12px] md:gap-[var(--gap-tight)]">
                ...
                <div>
                  <h3 className="text-[16px] md:text-[20px] font-medium leading-[24px] md:leading-[28px] text-[var(--color-text)] font-[var(--font-montaga)]">NVDA / NVIDIA Corp</h3>
                  <div className="flex items-center gap-[8px] mt-[2px]">
                    <span className="bg-[var(--color-signal-green)] text-white text-[10px] px-[8px] py-[2px] rounded-[2px]">High Conviction</span>
                    <span className="text-[11px] md:text-[12px] tracking-[0.5px] text-[var(--color-text-muted)]">Alert ID: #88321-X</span>
                  </div>
                </div>
              </div>
              <div className="text-right shrink-0 ml-[12px]">
                <p className="text-[20px] md:text-[24px] font-bold leading-[28px] md:leading-[32px] text-[var(--color-signal-green)] font-[var(--font-mono)]">BUY</p>
                <p className="text-[11px] md:text-[12px] tracking-[0.5px] text-[var(--color-text-muted)]">SEC Form 4</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[32px] md:gap-[40px] p-[20px] md:p-[40px]">
            <div>
              <p className="text-[13px] tracking-[0.5px] text-[var(--color-text-muted)] mb-[24px]">Transaction Data</p>
              {[["Insider","Jensen Huang (CEO)"],["Shares Purchased","240,000"],["Average Price","$130.15"],["Total Value","$2,298,580"]].map(([l,v])=>(
                <div key={l} className="flex justify-between py-[8px] border-b border-[var(--color-border-light)]">
                  <span className="text-[16px] leading-[24px] text-[var(--color-text-secondary)]">{l}</span>
                  <span className={`text-[16px] font-semibold leading-[24px] ${l==="Total Value"?"text-[var(--color-signal-green)]":"text-[var(--color-text)]"}`}>{v}</span>
                </div>
              ))}
            </div>
            <div>
              <p className="text-[13px] tracking-[0.5px] text-[var(--color-text-muted)] mb-[24px]">AI Analysis</p>
              <div className="bg-[var(--color-bg-alt)] p-[24px] space-y-[var(--gap-tight)]">
                <div>
                  <p className="text-[16px] font-semibold text-[var(--color-text)] mb-[4px]">CONTEXT</p>
                  <p className="text-[14px] leading-[20px] text-[var(--color-text)]">340% above historical average purchase size for this executive.</p>
                </div>
                <div className="border-t border-[var(--color-border)] pt-[var(--gap-tight)]">
                  <p className="text-[16px] font-semibold text-[var(--color-text)] mb-[4px]">HISTORICAL PERFORMANCE</p>
                  <p className="text-[14px] leading-[20px] text-[var(--color-text)]">Huang&apos;s last 3 purchases preceded an average 18% 60-day price gain.</p>
                </div>
                <div className="border-t border-[var(--color-border)] pt-[var(--gap-tight)]">
                  <p className="text-[16px] font-semibold text-[var(--color-text)] mb-[4px]">SENTIMENT SCORE</p>
                  <div className="flex items-center gap-[8px]">
                    ...
                    <span className="text-[14px] font-semibold text-[var(--color-signal-green)]">87/100</span>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <p className="text-[14px] leading-[20px] text-[var(--color-text-secondary)] text-center italic px-[32px] pb-[12px] pt-[25px]">
        ...
        <div className="flex flex-col md:flex-row items-center justify-center gap-[32px] md:gap-[60px] pt-[50px]">
          {[{v:"$4.2B",l:"Tracked Monthly"},{v:"2,847",l:"Alerts Sent (24h)"},{v:"17,325+",l:"Companies Monitored"}].map((m,i)=>(
            <div key={m.v} className="flex items-center gap-[60px]">
              <div>
                <p className="font-[var(--font-montaga)] text-[48px] font-normal leading-[40px] tracking-[0.5px] text-[var(--color-text)]">{m.v}</p>
                <p className="text-[14px] leading-[16px] text-[var(--color-text-secondary)] mt-[8px]">{m.l}</p>
              </div>
      ...
      <section className="w-full py-[var(--section-y-mobile)] px-[20px] md:pt-[var(--section-y)] md:pr-[68px] md:pb-[102px] md:pl-[68px] bg-[var(--color-bg-alt)]">
        <div className="max-w-[1144px] mx-auto px-[32px]">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] font-normal leading-[1.1] tracking-[0.5px] text-[var(--color-text)] mb-[40px] md:mb-[64px] text-center lg:text-left">See the Pattern</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-[var(--gap-items)] lg:gap-[var(--gap-cards)]">
            <div className="bg-white p-[40px] h-[400px]">
              <p className="text-[18px] leading-[28px] text-[var(--color-text)] mb-[32px]">Insider Buying vs Stock Price (S&P 500)</p>
              ...
              <div className="flex justify-between mt-[var(--gap-tight)]">{["Jan","Mar","May","Jul","Sep","Nov"].map(m=><span key={m} className="text-[10px] tracking-[0.5px] text-[var(--color-text-muted)]">{m}</span>)}</div>
            </div>
            <div className="bg-white p-[40px] h-[400px]">
              <p className="text-[18px] leading-[28px] text-[var(--color-text)] mb-[32px]">Monthly Insider Buying Activity ($B)</p>
              ...
              <div className="flex justify-between px-[var(--gap-tight)] mt-[var(--gap-tight)]">{["Jul","Aug","Sep","Oct","Nov","Dec","Jan"].map(m=><span key={m} className="text-[10px] tracking-[0.5px] text-[var(--color-text-muted)]">{m}</span>)}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="w-full py-[var(--section-y-mobile)] md:pt-[var(--section-y)] md:pb-[var(--section-y)] bg-[var(--color-primary-dark)] overflow-x-clip">
        <div className="max-w-[1136px] mx-auto px-[20px] md:px-[32px]">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] leading-[1.2] tracking-[0.5px] text-white text-center mb-[12px]">Deep Dive Reports</h2>
          <p className="text-[16px] leading-[24px] text-[var(--color-text-muted)] text-center mb-[37px]">Comprehensive stock analysis powered by SEC data and AI.</p>
            ...
            {REPORTS.map((r)=>(
              <div key={r.label} className={`bg-white p-[32px] shadow-[0px_1px_2px_rgba(0,0,0,0.05)] border border-black/10 ${r.best?"ring-2 ring-[var(--color-primary)]":""}`}>
                <div className="flex items-center gap-[12px] mb-[24px]">
                  ...
                  <span className="text-[16px] font-medium tracking-[1px] text-[var(--color-text)] font-[var(--font-mono)]">{r.label}</span>
                </div>
                <h3 className="font-[var(--font-montaga)] text-[26px] leading-[28px] tracking-[1px] text-[var(--color-text)] mb-[4px]">{r.title}</h3>
                <p className="text-[14px] leading-[20px] tracking-[1px] text-[var(--color-primary)] mb-[33px]">{r.pages}</p>
                <ul className="space-y-[var(--gap-tight)] mb-[40px]">
                  {r.features.map(f=>(<li key={f} className="flex items-center gap-[12px]"><svg className="w-[11px] h-[8px] shrink-0" viewBox="0 0 11 8"><path d="M1 4l3 3L10 1" stroke="#006d34" strokeWidth="2" fill="none"/></svg><span className="text-[14px] leading-[20px] text-[var(--color-text)]">{f}</span></li>))}
                </ul>
                <p className="font-[var(--font-montaga)] text-[48px] leading-[36px] tracking-[1px] text-[var(--color-text)] mb-[8px]">{r.price}</p>
                <p className="text-[14px] font-light tracking-[1px] text-[var(--color-text-muted)] mb-[24px]">*one-time payment</p>
            ...
            {REPORTS.map((r, ri)=>(
              <div key={r.label} className={`bg-white p-[28px] shadow-[0px_1px_2px_rgba(0,0,0,0.05)] border border-black/10 min-w-[280px] max-w-[300px] shrink-0 snap-center ${ri===REPORTS.length-1?"mr-[20px]":""} ${r.best?"ring-2 ring-[var(--color-primary)]":""}`}>
                <div className="flex items-center gap-[12px] mb-[20px]">
                  <div className="w-[32px] h-[32px] bg-[var(--color-bg-alt)] rounded" />
                  <span className="text-[14px] font-medium tracking-[1px] text-[var(--color-text)] font-[var(--font-mono)]">{r.label}</span>
                </div>
                <h3 className="font-[var(--font-montaga)] text-[22px] leading-[26px] tracking-[1px] text-[var(--color-text)] mb-[4px]">{r.title}</h3>
                <p className="text-[13px] leading-[18px] tracking-[1px] text-[var(--color-primary)] mb-[24px]">{r.pages}</p>
                <ul className="space-y-[12px] mb-[28px]">
                  {r.features.map(f=>(<li key={f} className="flex items-center gap-[10px]"><svg className="w-[11px] h-[8px] shrink-0" viewBox="0 0 11 8"><path d="M1 4l3 3L10 1" stroke="#006d34" strokeWidth="2" fill="none"/></svg><span className="text-[13px] leading-[18px] text-[var(--color-text)]">{f}</span></li>))}
                </ul>
                <p className="font-[var(--font-montaga)] text-[36px] leading-[32px] tracking-[1px] text-[var(--color-text)] mb-[6px]">{r.price}</p>
                <p className="text-[12px] font-light tracking-[1px] text-[var(--color-text-muted)] mb-[20px]">*one-time payment</p>
      ...
      <section className="w-full py-[var(--section-y-mobile)] md:pt-[var(--section-y)] md:pb-[var(--section-y)] bg-[var(--color-bg-alt)]">
        <div className="max-w-[1216px] mx-auto px-[20px] md:px-[32px]">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] leading-[1.1] tracking-[0.5px] text-[var(--color-text)] text-center mb-[40px] md:mb-[80px]">Simple Pricing</h2>
          <div className="max-w-[1024px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-[var(--gap-items)]">
            {PLANS.map((p)=>(
              <div key={p.name} className={`bg-white p-[48px] relative ${p.border}`}>
                ...
                <h3 className="font-[var(--font-montaga)] text-[32px] leading-[32px] tracking-[1px] text-[var(--color-text)] mb-[8px]">{p.name}</h3>
                <p className="text-[14px] leading-[20px] tracking-[1px] text-[var(--color-text-secondary)] mb-[32px]">{p.desc}</p>
                <div className="flex items-baseline mb-[48px]">
                  <span className="font-[var(--font-montaga)] text-[48px] leading-[48px] tracking-[0.5px] text-[var(--color-text)]">{p.price}</span>
                  <span className="text-[16px] leading-[24px] text-[var(--color-text-muted)] ml-[4px]">/mo</span>
                </div>
                <ul className="space-y-[var(--gap-tight)] mb-[48px]">
                  {p.features.map(f=>(<li key={f} className="flex items-center gap-[12px] text-[14px] leading-[20px] text-[var(--color-text)]"><div className="w-[11px] h-[11px] rounded-full border-2 border-[var(--color-signal-green)] flex items-center justify-center"><div className="w-[5px] h-[5px] rounded-full bg-[var(--color-signal-green)]"/></div>{f}</li>))}
                </ul>
      ...
      <section className="w-full py-[var(--section-y-mobile)] md:pt-[var(--section-y)] md:pb-[var(--section-y)] bg-white">
        <div className="max-w-[800px] mx-auto px-[20px] md:px-[32px]">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] leading-[1.1] tracking-[0.5px] text-[var(--color-text)] text-center mb-[40px] md:mb-[64px]">Frequently Asked Questions</h2>
            ...
            {FAQS.map((faq,i)=>(
              <div key={i} className="border-b border-[var(--color-border-light)] pb-[24px]">
                <button onClick={()=>setOpenFaq(openFaq===i?null:i)} className="w-full flex items-center justify-between">
                  <span className="text-[18px] leading-[28px] text-[var(--color-text)] text-left">{faq.q}</span>
                  <svg className={`w-[12px] h-[12px] shrink-0 ml-[var(--gap-tight)] text-[var(--color-text-muted)] transition-transform ${openFaq===i?"rotate-45":""}`} viewBox="0 0 12 12"><path d="M6 0v12M0 6h12" stroke="currentColor" strokeWidth="2"/></svg>
                </button>
                {openFaq===i&&<p className="mt-[var(--gap-tight)] text-[16px] leading-[26px] text-[var(--color-text-secondary)]">{faq.a}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="w-full bg-[var(--color-bg-dark)] py-[var(--section-y-mobile)] md:py-[var(--section-y-hero)]">
        <div className="max-w-[1216px] mx-auto px-[32px] text-center">
          <h2 className="font-[var(--font-montaga)] text-[39px] md:text-[var(--text-title)] leading-[1.2] tracking-[0.5px] text-white mb-[var(--gap-tight)]">You&apos;ll know in 60 seconds.</h2>
          <p className="text-[20px] leading-[28px] text-[var(--color-text-muted)] max-w-[672px] mx-auto mb-[32px]">Don&apos;t wait for the morning news. Join thousands of sophisticated investors receiving institutional-grade insider data delivered in real-time.</p>
